...
func create_item():
	...
	var pickup: Pickup = preload("res://lessons/pick_up.tscn").instantiate()
	pickup.item = item
	add_child(pickup)
	
	var randomized_angle := randf_range(0.0, 2.0 * PI)
	var randomized_direction := Vector2(1.0, 0.0).rotated(randomized_angle)
	var land_position := randomized_direction * randf_range(60.0, 120.0)
	...
	var jump := randf_range(30.0, 80.0)
	tween.tween_property(pickup, "position:y", land_position.y  -jump, HALF_FLIGHT_TIME)
